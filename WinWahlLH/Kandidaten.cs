...
namespace WinWahlLFH
{
    public partial class Kandidaten : Form
    {
            ...
            if (KandidatenListBox.SelectedItem != null)
            {
                ...
                foreach (Kandidat k in Program.alleKandidaten)
                {
                    if (k.getId() == id)
                    {
                        ...
                        KandidatNameBox.Text = kandidat.getName();
        ...
        {
            if (KandidatNameBox.Text != "" && KandidatAlterBox.Text != "" && KandidatBerufBox.Text != "")
            {
                if (KandidatenIdBox.Text == "")
                {
                    Program.alleKandidaten.Add(new Kandidat(Program.alleKandidaten.Count() + 1, KandidatNameBox.Text, Convert.ToInt16(KandidatAlterBox.Text), KandidatBerufBox.Text));
                    KandidatNameBox.Text = "";
                    KandidatAlterBox.Text = "";
                    KandidatBerufBox.Text = "";
                    KandidatenIdBox.Text = "";
                    ...
                    try
                    {
                        foreach (Kandidat kandidat in Program.alleKandidaten)
                        {
                            if (kandidat.getId() == Convert.ToInt16(KandidatenIdBox.Text))
                            {
                                ...
                                kandidat.setName(KandidatNameBox.Text);
        ...
        private void KandidatNameBox_TextChanged(object sender, EventArgs e)
        {
